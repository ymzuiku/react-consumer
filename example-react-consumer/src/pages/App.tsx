  ...
  return (
    <div style={{ background: '#fff' }}>
      <header>
        ...
        <Consumer subscribe={s => [s.status['/App']]}>{param => <h4>Param: {JSON.stringify(param)} </h4>}</Consumer>
        <button onClick={dispatchs.addNumber}>add number</button>
        <button onClick={() => routeMap.push('/User', { dog: 'wangwang', cc: ['123', 33, 'aaa'] })}>
          Go User Page
        </button>
        <button onClick={() => routeMap.push('/InfiniteList')}>Go To InfiniteList Page</button>
      </header>
      <section>
        <Consumer subscribe={s => [s.user.info.num]}>{num => <div>get:{num}</div>}</Consumer>
      </section>
    </div>
  );
};
